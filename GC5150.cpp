...
/*
    Written by Guitar Chad (GuitarChad#5150)
*/

// Return current Date/Time independent of replay
// Borrwed from Kory Gill 'New Study Template' (https://github.com/korygill/technical-analysis)
...
SCSFExport scsf_AutoClearRecentBidAskVolume(SCStudyInterfaceRef sc)
{
    ...
    if (sc.SetDefaults)
    {
        ...
        sc.StudyDescription = "This study automatically clears the 'Recent Bid/Ask Volume' for the symbol of the chart at specified session times. Option to also include 'Current Traded Bid/Ask Volume' when auto clearing.";
        sc.AutoLoop = 0;
        sc.GraphRegion = 0;
    ...
    // Run code, clear, set cleared flat. If we run again in the same time frame (only down to seconds) it won't trigger again
